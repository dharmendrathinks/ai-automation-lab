...
import { auditEvents, automationRuns, customers, outboxEvents, tickets } from './db/schema.js';

export async function createTicket(db: Database, input: CreateTicket, now = new Date()) {
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.insert(automationRuns).values({ id: runId, ticketId, phase: 'pending', mode: 'FIXTURE MODE', createdAt: now });
    ...
    return { ticketId, runId, eventId, phase: 'pending', mode: 'FIXTURE MODE' };
  });
}
