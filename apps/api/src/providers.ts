import { codexProcess, providerFailure, waitForTurn } from './codex-process.js';
...
function fixtureDecision(
  ...
  if (!customerResolved)
    return triageDecisionSchema.parse({
      ...
      evidenceRefs: ['ticket.customerRef'],
      unresolvedIssues: ['customer_identity'],
      reason: 'The synthetic customer reference did not resolve.',
    });
  if (text.includes('charged twice') || text.includes('duplicate charge'))
    return triageDecisionSchema.parse({
      schemaVersion: 1,
      category: 'billing',
      intent: 'duplicate_charge',
      priority: 'high',
      recommendedAction: 'investigate_refund',
      needsHumanReview: true,
      ambiguity: 'clear',
      evidenceRefs: ['ticket.message', 'customer.payments'],
      unresolvedIssues:
        text.includes('account') || text.includes('free')
          ? ['account_plan_mismatch']
          : [],
      reason:
        'The message reports a possible duplicate charge; deterministic billing policy must verify it.',
    });
  if (text.includes('download') && text.includes('invoice'))
    return triageDecisionSchema.parse({
      schemaVersion: 1,
      category: 'billing',
      ...
      ambiguity: 'clear',
      evidenceRefs: ['ticket.message', 'faq.invoice_download'],
      unresolvedIssues: [],
      reason: 'The request matches the supported invoice-download guidance.',
    });
  if (
    text.includes('charge') &&
    (text.includes('wrong') || text.includes('sort it'))
  )
    return triageDecisionSchema.parse({
      schemaVersion: 1,
      category: 'billing',
      ...
      needsHumanReview: false,
      ambiguity: 'missing_information',
      evidenceRefs: ['ticket.message'],
      unresolvedIssues: ['charge_identity'],
      reason: 'The message does not identify a specific billing problem.',
    });
  if (text.includes('account') || text.includes('plan'))
    return triageDecisionSchema.parse({
      ...
      needsHumanReview: true,
      ambiguity: 'clear',
      evidenceRefs: ['ticket.message', 'customer.subscription'],
      unresolvedIssues: ['account_plan_mismatch'],
      reason: 'Automatic account repair is outside the supported action set.',
    });
  return triageDecisionSchema.parse({
    schemaVersion: 1,
    category: 'general',
...
export class CodexProvider implements AIProvider {
  ...
  constructor(
    ...
    private readonly limits = { timeoutMs: 180_000, killGraceMs: 5_000 },
  ) {
    if (!isAbsolute(executable) || !isAbsolute(codexHome))
      throw new Error('Live Codex paths must be absolute.');
  }
  ...
  private async invoke(
    ...
    if (signal?.aborted) throw new Error('cancelled');
    if (Buffer.byteLength(JSON.stringify(context)) > 32 * 1024)
    ...
    try {
      await writeFile(
        schemaPath,
        JSON.stringify(z.toJSONSchema(triageDecisionSchema)),
      );
      if (Buffer.byteLength(prompt) > 32 * 1024) throw new Error('input_limit');
      const invoke = {
      ...
      await codexProcess({
        ...
        args: ['--version'],
        input: '',
        timeoutMs: Math.min(this.limits.timeoutMs, 10_000),
        line: (line) => {
          version += line;
        },
      });
      if (version.trim() !== 'codex-cli 0.155.0')
        throw new Error('unsupported_capability');
      let decision: TriageDecision | undefined,
        completed = false;
      let usage: ProviderResult['usage'] = null;
      await codexProcess({
        ...invoke,
        args,
        input: prompt,
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                Buffer.byteLength(item.text) > 64 * 1024
              )
                throw new Error('invalid_output');
              // Commentary may precede the final message; only a schema-valid final is usable.
              try {
                decision = triageDecisionSchema.parse(JSON.parse(item.text));
              } catch {
                decision = undefined;
              }
            }
            ...
            throw new Error(providerFailure(JSON.stringify(event)));
          } else if (
            !['thread.started', 'turn.started'].includes(String(event.type))
          )
            throw new Error('invalid_output');
        },
      });
      if (!completed || !decision) throw new Error('invalid_output');
      return {
        decision,
        provider: 'codex',
        model: this.model,
